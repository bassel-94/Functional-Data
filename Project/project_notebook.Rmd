...
A dimensionality reduction approach has always been useful to solve high dimensional multivariate data by projecting said data into a smaller, more managable feature space. From a functional data approach, this has proven to give accurate resutls when solving a scalar-on-function regression to solve the problem in this study which is to be able to detect the level of moisture in wheat grains from its NIR spectra. Indeed, based on its $R^2$ estimate, the problem at hand has been solved which might help reduce the cost of chemical analyses in future works. On the other hand, a basis expansion approach is more easily interpretable based on the fact that we do not project the data onto smaller dimensions. The downside, however, is that a careful choice of the basis expansion functions and their number has to be taken.
